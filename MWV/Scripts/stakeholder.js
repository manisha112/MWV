...
$('#stakeholderReports').on('change', 'select.choose_report', function () {
    ...
    switch ($(this).val()) {
        case 'customer':
            ...
            break;
        case 'agent':
            GetRecentbyReporttype("AgentReport");
            $('#searchAgents').show();
            $('#customersAgentsOptions').show();
            break;
        case 'order-book':
            GetRecentbyReporttype("OrderBookReport");
            $('#orderBook').show();
            break;
        case 'bd':
            GetRecentbyReporttype("BdReport");
            $('#bdReport').show();
            break;
        case 'fg':
            GetRecentbyReporttype("FGReport");
            $('#fgReport').show();
            break;
        case 'sourcing':
            GetRecentbyReporttype("SourcingReport");
           // $('#fgReport').show();
            break;
...
$('#stakeholderReports').on('click', 'a[href="#searchReports"]', function () {
    ...
    var fromdt = $('#fromDateYear').val() + "/" + $('#fromDateMonth').val() + "/" + $('#fromDateDay').val();
    var todt = $('#toDateYear').val() + "/" + $('#toDateMonth').val() + "/" + $('#toDateDay').val();
...
function GetAllrecentReports() {
    $.ajax({
        cache: false,
        type: "GET",
        url: "/Stakeholder/GetlastRecords/",
        contentType: "application/text; charset=utf-8",
        success: function (data) {
            $('#downloadAgentReport').html('');
...
//On Dropdown Recent
function GetRecentbyReporttype(reportName) {
    $('#reportResultsPanel').hide();
    $.ajax({
        cache: false,
        type: "GET",
        url: "/Stakeholder/GetRecentRecordsByReport?reportName=" + reportName,
        contentType: "application/text; charset=utf-8",
        success: function (data) {
            $('#downloadAgentReport').html('');
            $('#downloadAgentReport').show();
